...
class Player {
  constructor(context, isAI=false) {
    ...
    // Chaque joueur peut être en Game Over indépendamment
    ...
    this.score = 0;
    this.level = 1;
    this.scoreForNextLevel = 2000;  // +2000 par level
    // Max level = 10

    // Inventaire d'objets gagnés
    this.inventory = [];
  ...
  /**
   * Vitesse : base / level (jusqu'à 10).
   * => level 1 => 800 ms, level 2 => 400 ms, ...
   * => level 10 => 80 ms
   */
  getCurrentDropInterval() {
    const factor = Math.min(this.level, 10); // plafonner à 10
  ...
  update(deltaTime) {
    if (this.isGameOver) return; // si perdu, on ne fait plus rien
  ...
  handleSpecialRules(linesCleared) {
    ...
    // Check level up (jusqu'à 10)
    this.checkLevelUp();

    // Pause douceur ex. tous les 1000 pts
  ...
  activateBonus() {
    if (this.isGameOver) return; // en Game Over, plus rien
  ...
  draw() {
    if (this.isGameOver) return; // grille figée
...
document.addEventListener('keydown',(e)=>{
  if (playerHuman.isGameOver || !playerHuman.currentPiece) return;
  switch(e.code) {
    case 'ArrowLeft':
      playerHuman.pos.x--;
...
/****************************************************
 * IA AMÉLIORÉE (handleAI) + FIX en haut
 ****************************************************/
function handleAI(ai) {
  if (ai.isGameOver || !ai.currentPiece) return;
  ...
  // Tester 4 rotations
  for (let r=0; r<4; r++) {
    const rotated=rotateMatrix(originalShape,r);
    // Tester X
    for (let x=-2; x<COLS+2; x++) {
      ...
      if (collisionTest(ai.board,rotated,{x,y:yPos})) {
        continue;
      }
      // Placement possible
  ...
  // Si aucune position n'est possible => IA perd
...
function countHoles(tb){
  ...
  for(let x=0;x<COLS;x++){
    ...
    for(let y=0;y<ROWS;y++){
      ...
      else if(blockFound && tb[y][x]===0) holes++;
    }
  }
  return holes;
}
...
function detectRotation(curShape,origShape){
  for(let r=0;r<4;r++){
    const test=rotateMatrix(origShape,r);
    if(areMatricesEqual(test,curShape)){
      return r;
    }
  }
  return 0;
